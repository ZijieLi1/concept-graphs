'''
Live USB Record3D mapping. Same detection/mapping logic as rerun_realtime_mapping.py;
frames, K, and pose come from Record3DStream instead of a preprocessed dataset.
'''
...
def main(cfg: DictConfig):
    ...
    print("Connecting to Record3D now (after models loaded)...")
    app.connect_to_device(dev_idx=0)
    print("Waiting for first USB frame. Record3D must be streaming; no other USB client.")
    ...
    for frame_idx in trange(total_frames):
        ...
        s_rgb = s_depth = s_intrinsic_mat = s_camera_pose = None
        waits = 0
        while s_rgb is None and not stop_requested:
            s_rgb, s_depth, s_intrinsic_mat, s_camera_pose = app.get_frame_data()
            if s_rgb is None:
                waits += 1
                if waits == 1 or waits % 5 == 0:
                    print(
                        f"Waiting for USB frame ({waits}s). "
                        "Start Record3D USB streaming and stop any other stream client."
                    )
        if stop_requested:
            break
        ...
        cam_K = s_intrinsic_mat.cpu().numpy()[:3, :3]
        if frame_idx == 0:
            print(
                f"USB live camera  rgb={s_rgb.shape} depth={s_depth.shape}\n"
                f"K=\n{cam_K}\n"
